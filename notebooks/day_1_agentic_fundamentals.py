...
@app.cell
def _rubric_helper(repo_root, sys, text):


    notebooks_root = repo_root / "notebooks"
    _text = str(notebooks_root)
    if text not in sys.path:
...
def _rubric_surface(mo, render_daily_rubric_callout, repo_root):
    render_daily_rubric_callout(mo, day="01", repo_root=repo_root)
    return
...
def _fixture_walkthrough(
    ...
    _candidate = load_candidate(repo_root / "fixtures" / "locale_mismatch" / "candidate.json")

    try:
        canonical = canonicalize_with_candidate(package, _candidate)
        result = mo.callout(
            mo.md(
                f"""
...
@app.cell
def _verification(mo, repo_root):
